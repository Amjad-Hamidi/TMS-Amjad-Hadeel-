...
const StyledAvatar = styled(Avatar)(({ theme }) => ({
  ...
  '&:hover': {
    transform: 'scale(1.12)', // Slightly increased scale
  ...
  '& .MuiAvatar-img': {
    ...
    width: '100%',
    height: '100%',
    borderRadius: '50%', // Ensure image itself is clipped if not perfectly fitting
  },
}));
...
const CompanyFeedback = () => {
  ...
  const fetchFeedbacksCallback = useCallback(async (pageToFetch, currentSearchTerm) => {
    ...
    try {
      ...
      if (currentSearchTerm) {
        params.append('search', currentSearchTerm); // Corrected parameter name to 'search'
      }
  ...
  // Fetch feedbacks when page or debounced search term changes
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAttachmentName("");
      setShowForm(false);
      // Refresh feedbacks to show the latest, potentially on the first page
      fetchFeedbacksCallback(1, ""); 
      setSearchTerm(""); // Clear search term after successful send
    } catch (err) {
      ...
      {showForm && (
        <StyledCard elevation={4} sx={{ p: { xs: 2, md: 3}, mb: 4, backgroundColor: 'white' }}>
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate>
            {/* Form Grid remains largely the same, ensure all fields are present */}
