...
import "../styles/Login.css"; // تأكد أن هذا الملف موجود
...
import logo from '../images/TMS Logo.png'; // تأكد أن المسار صحيح
import qrImage from "../images/QR Code-TMS.png"; // تأكد أن المسار صحيح
...
export default function Login() {
    ...
    // عناصر التنقل للـ AppBar (يمكن تعديلها حسب الحاجة)
    const navItems = [
    ...
    const handleLogin = async () => {
        ...
        try {
            // استخدام fetch كما في الكود الذي قدمته
            ...
            if (!response.ok) {
                ...
                setErrorMessage(errorData.message || "Login failed due to invalid credentials.");
            ...
            setErrorMessage("Login failed: " + (error.message || "An unexpected error occurred."));
    ...
    const handleKeyDown = (e) => {
        if (e.key === "Enter") {
            handleLogin(); // استدعاء handleLogin بدون تمرير 'e' لأن handleLogin لا تستخدمها
        }
    };
        ...
        <Box sx={{
            ...
            <Grid container sx={{ flexGrow: 1 }}>
                ...
                <Grid item xs={12} md={6} sx={{
                    ...
                    <Box sx={{ textAlign: 'center' }}>
                        ...
                        />
                        <Typography variant={isMobile ? 'h5' : 'h3'} fontWeight={700} color={darkMode ? "white" : "primary"} sx={{ mb: 2 }}> {/* <--- تطبيق Dark Mode */}
                            Welcome Back to TMS
                        </Typography>
                        <Typography variant="h6" color={darkMode ? "lightgray" : "text.secondary"} sx={{ maxWidth: 350, mx: 'auto' }}> {/* <--- تطبيق Dark Mode */}
                ...
                <Grid item xs={12} md={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', p: 2 }}>
                    <Paper elevation={isMobile ? 0 : 6} sx={{
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                            ...
                            <Typography variant="h4" fontWeight={700} color={darkMode ? "white" : "primary"}>Sign in</Typography> {/* <--- تطبيق Dark Mode */}
                        </Box>
                        ...
                        <TextField
                            ...
                            autoComplete="email"
                            required
                            InputLabelProps={{ style: { color: darkMode ? '#bbb' : 'inherit' } }} // <--- تطبيق Dark Mode
                            InputProps={{
                                ...
                                sx: {
                                    ...
                                    '& .MuiOutlinedInput-root': {
                                        backgroundColor: darkMode ? '#2d2d2d' : '#fff', // <--- تطبيق Dark Mode
                                        '& .MuiOutlinedInput-notchedOutline': { borderColor: darkMode ? '#555' : 'rgba(0, 0, 0, 0.23)' }, // <--- تطبيق Dark Mode
                                        '&:hover .MuiOutlinedInput-notchedOutline': { borderColor: darkMode ? theme.palette.primary.light : theme.palette.primary.main }, // <--- تطبيق Dark Mode
                                        '&.Mui-focused .MuiOutlinedInput-notchedOutline': { borderColor: darkMode ? theme.palette.primary.light : theme.palette.primary.main }, // <--- تطبيق Dark Mode
                                    },
                                }
                            }}
                            sx={{
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', flexBasis: '66.66%' }}>
                                <input
                                    ...
                                    style={{ marginRight: 8, accentColor: darkMode ? theme.palette.primary.main : 'auto' }} // <--- تطبيق Dark Mode
                                />
                                <Typography variant="body2" htmlFor="rememberMe" sx={{ fontSize: '1.1rem', color: darkMode ? '#bbb' : 'inherit' }}> {/* <--- تطبيق Dark Mode */}
                                    Remember me
                                </Typography>
                            </Box>
                            <Button variant="text" size="small" onClick={() => navigate('/forgetPassword')} sx={{
                                textTransform: 'none',
                                fontWeight: 600,
                                color: darkMode ? theme.palette.info.light : theme.palette.primary.main, // <--- تطبيق Dark Mode
                                "&:hover": {
                                    bgcolor: darkMode ? 'rgba(255,255,255,0.05)' : 'rgba(25,118,210,0.04)', // <--- تطبيق Dark Mode
                                }
                            }}>
                                Forgot password?
                            </Button>
                        ...
                        <Button
                            ...
                            size="large"
                            sx={{
                                mt: 3, mb: 1, borderRadius: 2, fontWeight: 600,
                                bgcolor: darkMode ? theme.palette.primary.dark : theme.palette.primary.main, // <--- تطبيق Dark Mode
                                '&:hover': { bgcolor: darkMode ? theme.palette.primary.main : theme.palette.primary.dark } // <--- تطبيق Dark Mode
                            }}
                        >
                            Sign in
                        </Button>

                        <Button variant="outlined" color="inherit" fullWidth startIcon={<Google />} sx={{
                            textTransform: 'none', borderRadius: 2, mb: 2,
                            borderColor: darkMode ? '#555' : 'rgba(0, 0, 0, 0.23)', // <--- تطبيق Dark Mode
                            color: darkMode ? '#eee' : 'inherit', // <--- تطبيق Dark Mode
                            '&:hover': { borderColor: darkMode ? '#777' : 'inherit', bgcolor: darkMode ? 'rgba(255,255,255,0.05)' : 'rgba(0, 0, 0, 0.04)' } // <--- تطبيق Dark Mode
                        }} onClick={() => alert('Google login not implemented.')}>
                            Sign in with Google
                        </Button>

                        <Typography variant="body2" sx={{ textAlign: 'center', mt: 1, color: darkMode ? '#bbb' : 'inherit' }}> {/* <--- تطبيق Dark Mode */}
                            Don't have an account?{' '}
                            <Button variant="text" size="small" onClick={() => navigate('/register')} sx={{
                                textTransform: 'none',
                                fontWeight: 600,
                                color: darkMode ? theme.palette.info.light : theme.palette.primary.main, // <--- تطبيق Dark Mode
                                "&:hover": {
            ...
            >
                <Box sx={{
                    ...
                    <IconButton onClick={() => setLogoModalOpen(false)} sx={{
                        ...
                        boxShadow: darkMode ? "0 0 12px rgba(255,105,135,0.5)" : "0 0 8px rgba(244,67,54,0.3)", // <--- تطبيق Dark Mode
                        transition: "all 0.3s ease"
                    }}><CloseIcon /></IconButton>
                    <CardMedia component="img" src={logo} alt="Full TMS Logo" sx={{
                        borderRadius: 3,
                        maxWidth: "100%",
                        maxHeight: "80vh",
                        boxShadow: darkMode ? "0 0 25px rgba(144,202,249,0.6)" : "0 0 25px rgba(0,0,0,0.2)", // <--- تطبيق Dark Mode
            ...
            <Box id="contact" component="footer" sx={{
                py: 3,
                textAlign: "center",
                background: darkMode ? "linear-gradient(135deg, #1c1c1c, #3a3a3a)" : "linear-gradient(135deg, #a7d9ff, #c6e7ff)", // <--- تطبيق Dark Mode
                ...
                <Box sx={{ display: "flex", justifyContent: "center", gap: 3, fontSize: 30, mb: 2 }}>
                    <IconButton href="mailto:tms.contactus1@gmail.com" sx={{ color: darkMode ? "#fff" : "#1976d2", animation: "iconPulse 2s infinite .1s" }}><EmailIcon /></IconButton> {/* <--- تطبيق Dark Mode */}
                    <IconButton href="https://www.linkedin.com/in/amjad-hamidi/" target="_blank" sx={{ color: darkMode ? "#fff" : "#1976d2", animation: "iconPulse 2s infinite .5s" }}><LinkedInIcon /></IconButton> {/* <--- تطبيق Dark Mode */}
                    <IconButton href="https://www.instagram.com/amjada871/" target="_blank" sx={{ color: darkMode ? "#fff" : "#1976d2", animation: "iconPulse 2s infinite .7s" }}><InstagramIcon /></IconButton> {/* <--- تطبيق Dark Mode */}
                    <IconButton href="https://www.facebook.com/AmjadHamidi01/" target="_blank" sx={{ color: darkMode ? "#fff" : "#1976d2", animation: "iconPulse 2s infinite .9s" }}><FacebookIcon /></IconButton> {/* <--- تطبيق Dark Mode */}
                </Box>
                <Box sx={{ display: "flex", justifyContent: "center", gap: 2, mb: 2 }}>
                    <Button
                        ...
                        sx={{
                            ...
                            "&:hover": { backgroundColor: darkMode ? "rgba(255,255,255,0.1)" : "rgba(25,118,210,0.1)" }, // <--- تطبيق Dark Mode
                            transition: "0.3s"
                        }}
                    >
                        Share Website
                ...
                {showQR && (
                    <Box sx={{ mt: 2, animation: "fadeIn 0.6s ease-in" }}>
                        <CardMedia component="img" src={qrImage} alt="QR Code" sx={{
                            ...
                            boxShadow: darkMode ? "0 0 12px rgba(144,202,249,0.4)" : "0 0 10px rgba(0,0,0,0.2)", // <--- تطبيق Dark Mode
                            "&:hover": {
                                transform: "scale(1.1)",
                                boxShadow: darkMode ? "0 0 25px rgba(144,202,249,0.6)" : "0 0 20px rgba(0,0,0,0.3)" // <--- تطبيق Dark Mode
                            },
                    ...
                    >
                        <Box component="span" sx={{
                            ...
                            <svg width="50" height="25" viewBox="0 0 100 30" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginLeft: 4 }}>
                                <path d="M0 20 C20 0, 30 30, 50 20 S80 0, 100 20" stroke={darkMode ? "#90caf9" : "#1976d2"} strokeWidth="1.2" fill="transparent" /> {/* <--- تطبيق Dark Mode */}
                            </svg>
                        </Box>
                    </Typography>
